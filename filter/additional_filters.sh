...
function usage {
    ...
    echo >&2 "       column_in_maf [--info] -c column_name -d description /path/to/input.maf sample_ID filter_name -i /path/to/input.vcf -o output.vcf: adds filter/info to variant from column in input.maf"
    echo >&2 "       sex {male|female} /path/to/input.vcf: filters Y-chrom calls if female"
